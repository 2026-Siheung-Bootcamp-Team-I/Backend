...
class DetectionTopologyTest {
    ...
    private Event process(String host, String proc, String parent, long ts) {
        return new Event(host, Event.TYPE_PROCESS, ts, proc, parent, proc, null, 0, null, null, "tenant-a");
    }

    /** cmdline 을 직접 주는 process 이벤트. R2 는 실행 경로를 본다. */
    private Event processFrom(String host, String proc, String cmdline, long ts) {
        return new Event(host, Event.TYPE_PROCESS, ts, proc, "explorer.exe", cmdline, null, 0, null, null, "tenant-a");
    }

    private Event network(String host, int destPort, long ts) {
        return new Event(host, Event.TYPE_NETWORK, ts, null, null, null, "203.0.113.9", destPort, null, null, "tenant-a");
    }
