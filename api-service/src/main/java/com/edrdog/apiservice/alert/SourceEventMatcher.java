 ...
 * 판별자(matched 마지막 줄 → rule_id 순)로 후보를 좁힌 뒤 그 안에서만 시각으로 고른다.
 */
// 판별자 없이 시각만으로 고르면 초당 십여 건 속에서 제비뽑기가 된다. 틀린 원인을 확신에 차서 보여 주는 건 안 보여 주는 것보다 나쁘다.
public final class SourceEventMatcher {

    // 후보를 훑는 앞뒤 폭. 넓히면 같은 판별자를 만족하는 반복 실행에서 엉뚱한 회차를 고른다.
    public static final long WINDOW_MS = 5000;

    // 룰이 어떤 type 의 이벤트로 완성되는지(detector Rules). 룰이 바뀌면 여기도 같이 바꿔야 원본 이벤트가 안 끊긴다.
    ...
    /** detector Rules.summary 형식을 되짚어 트리거 이벤트를 짚는 조건을 만든다. */
    // 형식이 다르면 null 로 다음 판별자에 넘긴다. 파싱 실패를 예외로 다루면 요약 형식이 바뀔 때 상세가 통째로 죽는다.
    ...
    // detector 가 트리거를 matched 마지막에 담는다는 규약(Rules.alertOf)에 기댄다. 순서가 바뀌면 엉뚱한 이벤트를 짚는다.
    ...
    /** "process <name> (parent <parent>)" 의 부모. */
    // 부모 값 자체에 괄호가 있을 수 있어 닫는 괄호는 뒤에서 찾는다. 앞에서 찾으면 부모명이 잘린다.
