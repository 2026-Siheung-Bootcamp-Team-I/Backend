...
#
# env 를 끼우는 이유: sudo VAR=값 명령 으로 쓰면 sudoers 의 NOPASSWD 가 적용되지 않아 비밀번호를 묻는데, 오라클 Ubuntu 이미지는 그 계정에 비밀번호가 없어서 답할 수 없다.
#
# 하는 일:
...
#
# 안 하는 일(사람이 해야 한다):
#   - 오라클 콘솔의 보안 목록(Security List) 에 80/443/30443 을 여는 것. 인스턴스 밖의 설정이라 여기서 건드릴 수 없다(아래에서 다시 알려준다).
#   - Infisical 로그인. 로그인돼 있으면 edrdog-secrets 를 자동으로 넣고, 아니면 무엇이 안 뜨는지 알려준다.
set -euo pipefail
...
# --- 1. 호스트 방화벽 ---------------------------------------------------------
# 오라클 이미지는 INPUT 사슬 끝에 REJECT 가 박힌 채로 나온다. 콘솔의 보안 목록만 열고 여기를 안 열면 밖에서는 그냥 타임아웃으로 보이는, 원인을 짚기 가장 어려운 자리다.
...
# --- 2. k3s -------------------------------------------------------------------
step "2/8 k3s"
# traefik 을 끄고 깐다. k3s 는 기본으로 traefik 을 깔아 호스트 80/443 을 잡는데, 그러면 Caddy 가 "address already in use" 로 못 뜬다(우리는 Caddy 로 프록시한다).
...
# --- 4. Caddy -----------------------------------------------------------------
step "4/8 Caddy"
# 80 을 다른 웹서버가 잡고 있으면 Caddy 가 "address already in use" 로 못 뜬다. 실제 인스턴스에 nginx 가 올라와 있어 아래 restart 가 조용히 실패했었다.
...
# 이름이 agent-tls 여야 한다. 매니페스트가 optional:true 로 참조해서, 틀리면 커넥터가 꺼진 채로 파드는 멀쩡히 떠서 정상으로 보이는데 에이전트만 못 붙는다.
...
# --- 7. edrdog-secrets --------------------------------------------------------
# api / collector / responder 가 envFrom 으로 이걸 요구해서 없으면 셋 다 CreateContainerConfigError 로 멈춘다. alert / detector / archiver 는 참조하지 않아 멀쩡히 뜨는데, 절반만 도는 그 모습이 원인을 짚기 가장 어렵다(실제로 여기서 배포가 막혔다).
...
if [[ -d "$REPO_DIR/.git" ]]; then
  ...
  # 목록을 적지 않고 k8s/ 를 훑는다. 파일을 추가할 때 여기도 같이 고쳐야 하면 잊게 되고, 그러면 서버에만 빠진 채로 지나간다(CD 도 같은 규칙으로 고른다, .github/workflows/cd.yml).
