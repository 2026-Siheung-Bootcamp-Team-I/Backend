...
// ClientHello 는 TLS 핸드셰이크 첫 메시지에서 뽑은 값이다. 인증서는 담지 않는다.
...
// 입력은 ClientHello 레코드가 통째로 담긴 바이트여야 한다. 세그먼트를 모으는 일은 Assembler 가 한다.
// SNI 가 없어도 true 를 준다. 여기서 false 를 주면 "TLS 였다" 는 사실까지 잃는다.
func ParseClientHello(payload []byte) (ClientHello, bool) {
	...
	for !e.empty() {
		...
		case extSupportedVersions:
			// 이 분기를 지우면 앞의 버전 필드가 1.2 로 고정돼 모든 최신 클라이언트가 TLS 1.2 로 기록된다.
...
func parseSNI(data []byte) (string, bool) {
	...
	for !r.empty() {
		...
		if nameType == sniHostName {
			// SNI 도 소문자로 낮춘다. DNS 이름과 같은 규칙으로 정규화해야 한 도메인으로 묶인다.
...
// 한 레코드에 여러 메시지가, 한 패킷에 여러 레코드가 들어갈 수 있어 양쪽 모두 순회한다.
// 레코드나 메시지가 이 바이트 안에서 끝나지 않으면 false 다.
...
// reader 는 경계 검사를 한곳에 모은 바이트 커서다. 읽기는 전부 이 타입을 거친다.
// 손으로 인덱싱하면 검사를 한 군데 빠뜨리기 쉽고, 그 한 군데가 조작된 패킷에서 panic 이 된다.
