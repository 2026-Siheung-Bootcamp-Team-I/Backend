...
//
// 서버의 event-schema/src/main/proto/edrdog/v1/event.proto 와 손으로 맞춰 둔 스키마 계약이다(#182).
// 이 구간은 검증 전 원본이라 JSON 을 유지한다. 서버는 검증·정규화 뒤부터 Protobuf 로 바꿔 흘린다.
// 필드명이나 json 태그를 바꾸면 수집 API 가 조용히 못 읽는다(에이전트까지 Protobuf 로 넓히면 그때 사라질 위험이다).
//
