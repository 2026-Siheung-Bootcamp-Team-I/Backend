...
// Buffer 는 센서와 전송 사이의 경계 있는 큐다.
//
// 서버가 죽어 있거나 네트워크가 끊긴 동안에도 센서는 계속 이벤트를 낸다. 경계가 없으면
// 그동안 에이전트 메모리가 무한정 늘어난다. 그래서 용량을 넘으면 가장 오래된 것부터 버린다.
// 최신 행위가 탐지에 더 쓸모 있으므로 앞을 버리는 쪽이 맞다.
//
